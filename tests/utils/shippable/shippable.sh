...
if [ "${script}" != "sanity" ] && [ "${script}" != "units" ] && [ "${test}" != "sanity/extra" ]; then
    ...
    retry git clone --depth=1 --single-branch --single-branch https://github.com/linux-system-roles/rhc.git "${ANSIBLE_COLLECTIONS_PATH}/aansible_collections/linux-system-roles/rhc"
