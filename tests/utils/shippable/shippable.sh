...
if [ "${script}" != "sanity" ] && [ "${script}" != "units" ] && [ "${test}" != "sanity/extra" ]; then
    ...
    retry git clone --depth=1 --single-branch --single-branch https://github.com/linux-system-roles/rhc.git "${ANSIBLE_COLLECTIONS_PATH}/ansible_collections/fedora/linux_system_roles/rhc"
